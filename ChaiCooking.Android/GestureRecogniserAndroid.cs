...
[assembly: ExportRenderer(typeof(Gestures), typeof(GesturesRecogniser))]

namespace ChaiCooking.Droid
{
    public class GesturesRecogniser : ViewRenderer
    {
        ...
        public GesturesRecogniser(Context context)
            : base(context)
        {
        ...
        protected override void OnElementChanged(ElementChangedEventArgs<Xamarin.Forms.View> e)
        {
            ...
            var cGestures = (Gestures)e.NewElement;
            ...
            if (e.OldElement == null)
            {
                ...
                _listener.OnTap = (x, y) => cGestures.OnTap(x, y);

                if (cGestures.LongTapEnabled)
                {
                    _listener.OnLongTap = () => cGestures.OnLongTap();
                }

                _listener.OnTouchBegan = (x, y) => cGestures.OnTouchBegan(x, y);
                _listener.OnTouchEnded = (x, y) => cGestures.OnTouchEnded(x, y);

                if (cGestures.SwipeEnabled)
                {
                    _listener.OnSwipeLeft = () => cGestures.OnSwipeLeft();
                    _listener.OnSwipeRight = () => cGestures.OnSwipeRight();
                    _listener.OnSwipeUp = () => cGestures.OnSwipeUp();
                    _listener.OnSwipeDown = () => cGestures.OnSwipeDown();
                }

                if (cGestures.DragEnabled)
                { 
                    _listener.OnDrag = (x, y) => cGestures.OnDrag(x, y);
                }
            }
        }
